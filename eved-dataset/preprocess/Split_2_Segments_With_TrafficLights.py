...
# === 处理单个文件 ===
def process_file(file_path, traffic_gdf, segment_len=50.0, buffer_m=25):
...
for category, ids in categories.items():
    for vehid in ids:
        ...
        output_dir = f"../data/segmented_50m_eVED_trafficlight/{category}/{vehid}/"
        os.makedirs(output_dir, exist_ok=True)
        all_files = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if f.endswith(".csv")]

        with tqdm(total=len(all_files), desc=f"Processing {category} {vehid}", unit="file") as pbar:
            for f in all_files:
                seg_df = process_file(f, traffic_lights_gdf)
